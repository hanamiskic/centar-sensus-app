...
export class EventsService {
  ...
  /** Helper: izvrši fn unutar Angular injection konteksta */
  ...
  async listEvents(): Promise<any[]> {
    return this.inCtx(async () => {
      ...
      return snapshot.docs.map((d) => {
        ...
        const datumVrijeme = raw?.toDate ? raw.toDate() : raw ? new Date(raw) : null;
  ...
  async setExtraCount(id: string, value: number): Promise<void> {
    // (može i bez konteksta, ali držimo konzistentno)
    return this.inCtx(async () => {
      const target = doc(this.db, 'events', id);
